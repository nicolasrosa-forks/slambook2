...
/**
 * @brief Brute-Force Matching two sets of descriptors
 * @param desc1 The first descriptor
 * @param desc2 The second descriptor
 * @param matches Matches of the two images
 */
...
void ComputeORB(const Mat &img, vector<KeyPoint> &keypoints, vector<DescType> &descriptors){
    ...
    for(auto &kp: keypoints){
        if(kp.pt.x < half_boundary || kp.pt.y < half_boundary ||
           kp.pt.x >= img.cols - half_boundary || kp.pt.y >= img.cols - half_boundary){ // outside
           bad_points++;
           descriptors.push_back({});  // Fills with "empty"
           continue;  // Skip current iteration.
        }

        // Calculate the moments m01 and m10
        float m01 = 0.0, m10= 0.0;
        for (int dx = -half_patch_size; dx < half_patch_size; dx++){  // [-8, 7]
            for (int dy = -half_patch_size; dy < half_patch_size; dy++){  // [-8, 7]
                uchar pixel = img.at<uchar>(kp.pt.y + dy, kp.pt.x+dx);  // I(y, x)
                m10 += dx*pixel;
                m01 += dy*pixel;
            }
        }

        // Calculate the direction of the feature point, θ = arctan(m01/m10)
